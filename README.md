...
**Проблема.** Когда работаешь с Claude Code на нескольких проектах параллельно, у тебя обычно 4+ окон VS Code открыто. В таскбаре они одинаковые. `Alt+Tab` показывает идентичные превью. Чтобы понять, в каком ты проекте, приходится читать заголовок — это ломает фокус и стоит секунды на каждое переключение.

**Решение.** Раскрасить «раму» каждого окна (title bar, activity bar, status bar, вкладки, пузыри сообщений) в свой цвет. Мозг быстро привыкает: *«красный — стратегия, зелёный — Claude API, синий — клиентский проект»*. `Alt+Tab` становится мышечной памятью, превью в таскбаре узнаются мгновенно.

**Содержимое** окна (редактор, notebook, чат Claude) **не красится** — только chrome. Код читается как обычно, подсветка синтаксиса не страдает.

**Использование.** Скажи Claude `«раскрась проект»` — он покажет палитру из 20 пресетов через `AskUserQuestion`, применит выбранный. Или сразу: `«раскрась в navy»`, `/colorize-project burgundy`, или со своим hex: `/colorize-project #2a4f3c`.
...
Дальше Claude сам подхватит скилл при следующем запуске.

**Поддерживает оба сценария:** обычная папка (пишет в `.vscode/settings.json`) и multi-root workspace (пишет в `.code-workspace`-файл, в его блок `"settings"`). Авто-детектит `.code-workspace`-файлы в папке и обновляет их тоже.
