...
description: Раскрашивает раму VS Code-окна в выбранный цвет, чтобы различать параллельные окна Claude Code. Триггеры — «раскрась проект», «/colorize-project», «покажи цвета», «поменяй цвет окна». Поддерживает обычные папки (.vscode/settings.json) И multi-root workspace-файлы (.code-workspace). 20 пресетов: forest, royal-blue, royal-purple, burgundy, amber, teal, deep-cyan, indigo, dark-magenta, olive, crimson, steel-blue, navy, rust, plum, pine, slate-purple, brick, dark-pine, charcoal-blue. Или кастомный hex.
---
...
4. **Запусти скрипт.** Два варианта:
   ```
   # Вариант A — работаем с папкой (auto-detect .code-workspace внутри)
   ...
   # Вариант B — работаем с конкретным .code-workspace файлом
   powershell -ExecutionPolicy Bypass -File "$env:USERPROFILE\.claude\skills\colorize-project\scripts\apply-colors.ps1" -Color <name-or-hex> -WorkspaceFile "<полный-путь-к-.code-workspace>"
   ```
   Можно передать и `-WorkspacePath`, и `-WorkspaceFile` одновременно — скрипт применит к обоим.
...
- `SKILL.md` — этот файл
- `scripts/apply-colors.ps1` — PowerShell-движок: лукап пресета → деривация оттенков → запись `settings.json` или `.code-workspace`

Изменения цветов или добавление новых пресетов — правь словарь `$presets` в начале `apply-colors.ps1` и при необходимости таблицу в этом SKILL.md.

## Platform

Currently Windows + PowerShell only. The color algorithm is portable; a bash equivalent for macOS / Linux would be straightforward — PRs welcome.
